...
const inputClass = 'h-14 rounded-2xl border border-[color:var(--slot4-border)] bg-[var(--slot4-page-bg)] px-4 text-base font-semibold text-[var(--slot4-page-text)] outline-none transition placeholder:text-[var(--slot4-soft-muted-text)] focus:border-[var(--slot4-accent-fill)] focus:bg-white'
const buttonClass = 'inline-flex h-14 items-center justify-center rounded-2xl bg-[var(--slot4-dark-bg)] px-6 text-xs font-black uppercase tracking-[0.2em] text-white transition hover:bg-[var(--slot4-accent-fill)] disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-2xl border px-4 py-3 text-sm font-bold ${status === 'success' ? 'border-emerald-800 bg-emerald-50 text-emerald-800' : 'border-red-700 bg-red-50 text-red-700'}`}>{message}</p> : null}
